...
const AppProviders = ({ children }) => {
  ...
  const addToCart = useCallback(
    (payload) => {
      ...
      const existingIndex = nextItems.findIndex(
        (cartItem) => lineKey(cartItem) === lineKey(item),
      );
  ...
  const updateProfile = async (id, dto) => {
    const updated = await apiClient.patch(`/user/${id}`, dto);
    if (updated) {
      storage.set("vendors_online_user", updated);
      setUser(updated);
    }
    return updated;
  };
