    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <section className="bg-white rounded-lg shadow-md p-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Configure core marketplace behavior and notifications.</p>
        </div>
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <input 
              ...
              type="number" 
              min="0" 
              onChange={(event) => updateSetting('defaultDeliveryFee', Number(event.target.value))} 
            />
          </div>
          <div className="space-y-8 space-x-4 pt-4 border-t border-gray-200">
            ...
            <label className="flex items-center space-x-3 cursor-pointer">
              <input 
                ...
                type="checkbox" 
                onChange={(event) => updateSetting('orderNotifications', event.target.checked)} 
              />
            ...
            <label className="flex items-center space-x-3 cursor-pointer">
              <input 
                ...
                type="checkbox" 
                onChange={(event) => updateSetting('disputeNotifications', event.target.checked)} 
              />
              <span className="text-sm font-medium text-gray-700">Dispute notifications</span>
            </label>
          </div>
          <button className="w-full bg-gray-900 hover:bg-gray-800 text-white font-semibold py-3 rounded-lg flex items-center justify-center space-x-2 transition-colors" type="submit"><Save size={18} /><span>Save Settings</span></button>
        </form>
      </section>
    </div>
  )
